...
class translation_evidention(osv.osv):
    ...
    def _get_total_evid_cards(self, cr, uid, ids, field_name, field_value, context=None):
        """
        returns number of card estimated before translation
        """
        res={}
        for evid in self.browse(cr, uid, ids):
            total = 0
            for doc in evid.document_ids :
                total += doc.cards_estm * len(doc.task_ids)
    ...
    _columns = {
        ...
        'ev_sequence':fields.char('Number', size=64, select=1),
        ...
        'total_cards':fields.function(_get_total_evid_cards, string='Est cards', type="float"),
        ...
        for job in jobs:
            if emp_id == job.employee_id.id:
                if job.work_type == 'lect':
                    if job.job_done:
                        return 'Lectoring finished'
                    elif job.job_stop :
                        return 'Lectoring paused'
                    else:
                        return 'Lectoring'
...
class translation_work(osv.Model):
    ...
    def _get_time_spent(self, cr, uid, ids, field_name, field_values, context=None):
        ...
        for job in self.browse(cr, uid, ids):
            if job.job_stop: 
                time_from=datetime.strptime(job.job_start, DATETIME_FORMAT)
                time_to = datetime.strptime(job.job_stop, DATETIME_FORMAT)
